...
|---|---|---|
| `if` | Expression | An expression to evaluate in order to decide if the job/step where it appears must be run. An exit code of `0` is trated as `true`. Anything else is `false`. The expression will be run on the default shell, which is `sh` for better compatibility with Alpine Linux containers. |
